...
const DEFAULT_SETTINGS = {
	...
	contactTemplatePath: 'Templates/Contact Template.md',
}

// Default Templater template used by "Import contacts from VCF". The importer
// injects each contact's data into window._ct; the template names the note
// "@First Last" and uses tp.file.move to drop it into the People folder.
const DEFAULT_CONTACT_TEMPLATE = `<%*
// Auto-generated by At People. Edit freely — values come from window._ct.
const ct = window._ct || {};
const peopleFolder = (ct.peopleFolder || "").replace(/[/]+$/, "");
const target = peopleFolder ? peopleFolder + "/" + tp.file.title : tp.file.title;
if (peopleFolder && tp.file.path !== target + ".md") {
  await tp.file.move(target);
}
-%>
---
<%* if (ct.phone) { %>phone: <% ct.phone %>
<%* } %>
<%* if (ct.email) { %>email: <% ct.email %>
<%* } %>
<%* if (ct.org) { %>company: <% ct.org %>
<%* } %>
<%* if (ct.title) { %>job title: <% ct.title %>
<%* } %>
<%* if (ct.bday) { %>birthday: <% ct.bday %>
<%* } %>
<%* if (ct.adr) { %>address: <% ct.adr %>
<%* } %>
<%* if (ct.url) { %>website: <% ct.url %>
<%* } %>
<%* if (ct.note) { %>note: <% ct.note %>
<%* } %>
<%* if (ct.photo) { %>photo: "<% ct.photo %>"
<%* } %>
---
<%* if (ct.photo) { %>!<% ct.photo %>
<%* } %>
`;
...
module.exports = class AtPeople extends Plugin {
	...
	// Parse vCard text and create one person note per contact using a Templater
	// template (see settings). Each contact's data is injected into window._ct,
	// the template names the note "@First Last" and moves it into the People
	// folder. Skips duplicates and contacts with no resolvable name.
	async importVcf(text) {
		const templaterPlugin = this.app.plugins.plugins['templater-obsidian']
		if (!templaterPlugin || !templaterPlugin.templater) {
			new Notice('Contact import requires the Templater plugin. Install and enable Templater, then set the Contact Template path in settings.')
			return
		}
		const templater = templaterPlugin.templater
		const templatePath = this.settings.contactTemplatePath
		const templateFile = this.app.vault.getAbstractFileByPath(templatePath)
		if (!templateFile) {
			new Notice('Contact Template not found at "' + templatePath + '". Use Settings > Contact import > Create / Update Contact Template.')
			return
		}
		...
		for (const c of contacts) {
			...
			let noteName = '@' + display
			noteName = noteName.replace(/[\\/:*?"<>|]/g, '').trim()
			if (!noteName) { skipped++; continue }
			...
			let photoLink = ''
			if (c.photo) {
				try { photoLink = await this.resolveContactPhoto(c, peopleFolder, noteName) } catch (e) { photoLink = '' }
			}
			window._ct = {
				first, last, name: display,
				phone: (c.fields.TEL || []).join('; '),
				email: (c.fields.EMAIL || []).join('; '),
				org: (c.fields.ORG || []).join('; '),
				title: (c.fields.TITLE || []).join('; '),
				bday: (c.fields.BDAY || []).join('; '),
				adr: (c.fields.ADR || []).join('; '),
				url: (c.fields.URL || []).join('; '),
				note: (c.fields.NOTE || []).join('; ').replace(/\s*\n\s*/g, ' '),
				photo: photoLink,
				peopleFolder,
			}
			try {
				await templater.create_new_note_from_template(templateFile, undefined, noteName, false)
				created++
			} catch (e) {
				new Notice('Failed to import ' + noteName + ': ' + (e && e.message ? e.message : e))
				skipped++
			} finally {
				delete window._ct
			}
	...
	// Save a contact's PHOTO to the vault and return a wiki-link to embed it.
	// Supports inline base64, data: URIs, and remote http(s) URLs.
	async resolveContactPhoto(c, peopleFolder, noteName) {
		const photo = c.photo
		let data = null
		let ext = (c.photoType || 'jpg').toLowerCase().replace('jpeg', 'jpg')
		if (/^https?:\/\//i.test(photo)) {
			return '[' + noteName + ' photo](' + photo + ')'
		}
	...
	// Create or overwrite the Templater Contact Template at the configured path.
	async createContactTemplate() {
		const path = this.settings.contactTemplatePath
		if (!path) { new Notice('Set a Contact Template path first.'); return }
		const content = DEFAULT_CONTACT_TEMPLATE
		const existing = this.app.vault.getAbstractFileByPath(path)
		if (existing) {
			try { await this.app.vault.modify(existing, content) } catch (e) { new Notice('Could not update template: ' + e.message); return }
			new Notice('Contact Template updated at ' + path)
		} else {
			const folder = path.split('/').slice(0, -1).join('/')
			if (folder) { try { await this.app.vault.createFolder(folder) } catch (e) { /* exists */ } }
			try { await this.app.vault.create(path, content) } catch (e) { new Notice('Could not create template: ' + e.message); return }
			new Notice('Contact Template created at ' + path)
		}
	}
}
...
class AtPeopleSettingTab extends PluginSettingTab {
	...
	display() {
		...
		new Setting(containerEl)
			.setName('Contact Template path')
			.setDesc(multiLineDesc([
				'Path (inside your vault) to a Templater template used for each imported',
				'contact. The note is named "@First Last" and Templater moves it into the',
				'People folder. Use the button below to create or update the template.'
			]))
			.addText(text => {
				text.setPlaceholder(DEFAULT_SETTINGS.contactTemplatePath)
					.setValue(this.plugin.settings.contactTemplatePath)
					.onChange(async (value) => {
						this.plugin.settings.contactTemplatePath = value.trim()
						await this.plugin.saveSettings()
					})
				text.inputEl.style.width = '100%'
			})
		new Setting(containerEl)
			.setName('Create / Update Contact Template')
			.setDesc('Writes the default Templater template to the path above (overwrites if it already exists).')
			.addButton(button => button
				.setButtonText('Create / Update')
				.onClick(() => this.plugin.createContactTemplate())
			)
		new Setting(containerEl)
